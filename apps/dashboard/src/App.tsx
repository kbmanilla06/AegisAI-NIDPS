...
type PreventionPolicy = {
  ...
  limitations: string;
  false_capability_flags: Record<string, boolean>;
};
      ...
      {!auth ? (
        ...
        <>
          ...
          <div className="grid">
            ...
            {can("prevention:read") && (
              <section className="panel" aria-labelledby="prevention-title">
                <h2 id="prevention-title">Prevention (simulation only)</h2>
                <p role="note"><strong>
                  SIMULATION ONLY. Prevention previews and records what a defensive action WOULD
                  be, never performs one: no firewall, network, host, or traffic change occurs, and
                  OS firewall state is unchanged. Real or lab enforcement is not present.
                </strong></p>
                <p>
                  Lifecycle: a request is drafted, its 13 policy gates are evaluated (fail-closed),
                  a safe structured preview is recorded (never an executable command), then a
                  <em> simulated</em> execution and rollback are stored. No enforcement authority.
                </p>
                <ul>
                  {preventionPolicies.map((policy) => (
                    <li key={policy.id}>
                      {policy.name} v{policy.version} · {policy.lifecycle} · max duration{" "}
                      {policy.max_duration_seconds}s · hash {policy.definition_hash.slice(0, 12)}…
                      {" · "}
                      <span data-testid="prevention-false-flags">
                        real_prevention={String(policy.false_capability_flags.real_prevention)} ·
                        enforcement_authority=
                        {String(policy.false_capability_flags.enforcement_authority)}
                      </span>
                    </li>
                  ))}
                </ul>
                {preventionPolicies[0] && (
                  <p className="limitation">{preventionPolicies[0].limitations}</p>
                )}
              </section>
            )}
